...
class ModelBase(DeclarativeBase):
    id: Mapped[UUID] = mapped_column(
        UUID, primary_key=True, server_default=text("gen_random_uuid()"), comment="id"
    )


class TimeMixin(DeclarativeBase):
    created_at: Mapped[datetime] = mapped_column(
        DateTime, server_default=func.now(), comment="created_at"
    )
...
class RemarkBase(DeclarativeBase):
    remark: Mapped[str] = mapped_column(String(128), comment="remark")
